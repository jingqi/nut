 ...
 * @author jingqi
 * @date 2012-07-14
 * @last-edit 2012-07-14 15:28:50 jingqi
 */
...
public:
    /** 构造函数 */
    ptr(T *ptr = NULL) : m_ptr(ptr) {}

    /** 隐式类型转换 */
    template <typename U>
    ptr(const ptr<U>& r) : m_ptr(r.pointer()) {}

    /** 复制构造函数(因为是特化模板，故必须放在上述模板函数的后面) */
    ptr(const ptr<T>& r) : m_ptr(r.m_ptr) {}
...
public:
    ...
    inline bool operator==(const ptr<T>& r) const { return m_ptr == r.m_ptr; }
    inline bool operator!=(const ptr<T>& r) const { return m_ptr != r.m_ptr; }
