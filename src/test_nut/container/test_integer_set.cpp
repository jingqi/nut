...
class TestIntegerSet : public TestFixture
{
    virtual void register_cases() noexcept override
    {
        ...
        NUT_REGISTER_CASE(test_merged_remainder);
    ...
    void check_add_one_range(int first, int last, const char *expected)
    {
        ...
        NUT_TA(l.to_string() == "{[4,6],[12,15]}");

        l.add_value_range(first, last);
        NUT_TA(l.to_string() == expected);
    }
    ...
    void check_remove_one_range(int first, int last, const char *expected)
    {
        ...
        NUT_TA(l.to_string() == "{[4,6],[11,16],[26,28],[32,34]}");

        l.remove_value_range(first, last);
        if (l.to_string() != expected)
            cout << l.to_string() << " " << expected << endl;
        NUT_TA(l.to_string() == expected);
    }
    ...
    void test_intersect()
    {
        ...
        IntegerSet<int> rs = x & y;
        NUT_TA(rs.to_string() == "{[2,3],[5,10],13,[15,24]}");
    }
    ...
    void test_remainder()
    {
        ...
        IntegerSet<int> rs = x - y;
        NUT_TA(rs.to_string() == "{1,14}");
    }

    void test_merged_remainder()
    {
        IntegerSet<int> x;
        x.add_value_range(1, 3);
        x.add_value_range(5, 10);
        ...
        IntegerSet<int> rs = x ^ y;
        NUT_TA(rs.to_string() == "{1,4,[11,12],14,[25,100]}");
    }
