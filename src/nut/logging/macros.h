...
#define NUT_LOG_D(tag, fmt, ...) do { nut::LogManager::get_logger(tag)->log(nut::LL_DEBUG, NUT_SOURCE_LOCATION, (fmt), ##__VA_ARGS__); } while (false)
#define NUT_LOG_I(tag, fmt, ...) do { nut::LogManager::get_logger(tag)->log(nut::LL_INFO, NUT_SOURCE_LOCATION, (fmt), ##__VA_ARGS__); } while (false)
#define NUT_LOG_W(tag, fmt, ...) do { nut::LogManager::get_logger(tag)->log(nut::LL_WARN, NUT_SOURCE_LOCATION, (fmt), ##__VA_ARGS__); } while (false)
#define NUT_LOG_E(tag, fmt, ...) do { nut::LogManager::get_logger(tag)->log(nut::LL_ERROR, NUT_SOURCE_LOCATION, (fmt), ##__VA_ARGS__); } while (false)
#define NUT_LOG_F(tag, fmt, ...) do { nut::LogManager::get_logger(tag)->log(nut::LL_FATAL, NUT_SOURCE_LOCATION, (fmt), ##__VA_ARGS__); } while (false)
